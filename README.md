...
## 🔍 SQL Files — Detailed Breakdown

### `07_pd_calculation.sql` — Probability of Default
8 queries covering:
- Portfolio-level PD
- Monthly PD trend with 3-month moving average
- PD by region, income band, product type
- PD by utilization band (proves high utilization → high PD)
- PD lifetime curve by Months on Book (MOB)
- **Account-level 12-month forward PD score** (logistic approximation)
- IFRS 9 stage assignment based on PD

### `08_lgd_calculation.sql` — Loss Given Default
8 queries covering:
- Portfolio LGD and recovery rate
- LGD by product, income, region
- Monthly LGD trend (is recovery improving?)
- LGD distribution histogram
- Account-level LGD detail
- LGD vs credit limit comparison (collateral proxy)

### `09_ead_calculation.sql` — Exposure at Default
7 queries covering:
- Account-level EAD (CCF 50%, 75%, 100%)
- Portfolio EAD summary
- EAD by product, income, region
- **CCF sensitivity analysis** (how EAD changes with CCF assumption)
- Monthly EAD trend

### `10_expected_loss.sql` — EL = PD × LGD × EAD
5 queries covering:
- **Account-level EL calculation** with IFRS 9 stage
- Portfolio-level EL summary
- EL by IFRS 9 Stage (Stage 1/2/3)
- EL by product type
- **Monthly ECL Reserve Requirement** (provisioning schedule)

### `11_roll_rate_matrix.sql` — Roll Rate Analysis
6 queries covering:
- Full count-based migration matrix
- **Balance-weighted migration matrix** (for loss reserves)
- Monthly roll rate trend (is portfolio worsening?)
- Net roll rate (inflows vs outflows)
- Roll rate by region
- **3-month forward loss projection** using roll rates

### `12_vintage_analysis.sql` — Vintage Analysis
7 queries covering:
- Monthly vintage default rates
- **Cumulative default rate by vintage** (ever-defaulted)
- Vintage comparison at fixed MOBs (6/12/18/24 months)
- Vintage loss rate (balance-weighted)
- Vintage delinquency curves by income band
- Annual vintage scorecard (board reporting)
- **Early life delinquency** (first 6 MOBs — underwriting quality)
...
| Metric | Value |
|--------|-------|
| Portfolio PD (90+ DPD) | 18.27% |
| Portfolio LGD | 79.78% |
| Recovery Rate | 20.22% |
| Portfolio EAD (75% CCF) | 7.66 Billion |
| Total Expected Loss | 2.54 Billion |
| EL Rate (EL/EAD) | 33.11% |
| ML Model ROC-AUC | **0.990** |
| IFRS 9 Stage 1 | 24,238 accounts |
| IFRS 9 Stage 2 | 16,627 accounts |
| IFRS 9 Stage 3 | 9,135 accounts |
...
> • Delivered **6 Tableau dashboards** covering portfolio overview, delinquency trends, vintage curves, and expected loss
